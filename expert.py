...
class DoLogging():
    ...
    def do_logging(self, msg:str):
        ...
        logging.basicConfig(filename="logcpexpp.elg", level=logging.INFO)
        logging.info(msg)
...
def ask_for_question():
        ...
        try:
            ...
            print("###############################",
                ...
                "CP expert password change.. changing expert password on all CP boxes managed by mgmt server through APi call",
            ...
            password_hash=input("Enter expert password hash (you can generate one on CP system running -> /sbin/grub-md5-crypt):")
            ...
            if not user or not password or not server_ip:
                ...
                return {"connector":connector, "password_hash":password_hash}
...
def main():
    ...
    print ("Password hash is:{}".format(password_hash))
    ...
    mapping_list = [
        ...
        {"change-expert-password":"/bin/clish -s -c \'set expert-password-hash {}\' && /bin/echo $?".format(password_hash)},
    ...
    try:
        ...
        connector.logout()
        DoLogging().do_logging("\n main() - main end..")
        print ("More data in logcpexpp.elg, I am done here.")
